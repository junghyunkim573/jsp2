					...
					<tr>
						<td>
							...
							<c:if test="${pageBlock<pageNum}">
								<a href="${home}/Productlist.do?pageNum=${1}">[처음] </a>
							</c:if>
								
							<!-- 이전 링크 -->
							<c:if test="${startPage > pageBlock}">
								<a href="${home}/Productlist.do?pageNum=${startPage - pageBlock }">[이전] </a>
							</c:if>
								
							<!-- 페이징 링크 -->
							<c:forEach var="i" begin="${startPage }" end="${endPage }">
							<c:if test="${level == 5 }">
								<a href="${home}/Productlist.do?email=${session_email }&pageNum=${i }">[${i }] </a>
							</c:if>
							<c:if test="${level == 10 }">
							...
							<!-- 다음 링크 -->
							<c:if test="${endPage < pageCount }">
								<a href="${home}/Productlist.do?pageNum=${startPage + pageBlock}">[다음] </a>
							</c:if>
								
							<!-- 맨끝 링크 -->
							<c:if test="${startPage<pageCount}">
								<a href="${home}/Productlist.do?pageNum=${pageCount}">[맨끝] </a>
							</c:if>
							</c:if>
						</td>
					</tr>				
